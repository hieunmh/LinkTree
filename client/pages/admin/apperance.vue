                ...
                <div class="max-w-[750px] mx-auto pb-24">
                    <div id="ProfileSection">
                        ...
                        <div class="w-full bg-white rounded-3xl p-6">
                            ...
                            <div class="mt-4">
                                <TextInput placeholder="Profile Title" 
                                    v-model:input="name"
                                    inputType="text" :max="25"
                                    :error="errors && errors.name ? errors.name[0] : ''"
                                />
                            </div>

                            <textarea v-model="bio" rows="4" maxlength="80" placeholder="Bio"
                                @focus="isBioFocused = true" @blur="isBioFocused = false"
                                :class="isBioFocused ? 'border-gray-900' : ''"
                                class="w-full mt-4 bg-[#eff0eb] text-gray-800 border-2 text-sm border-[#eff0eb] 
                        ...
                        <div class="w-full bg-white rounded-3xl p-6">
                            <div class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-2 gap-4">
                                <div v-for="item in userStore.colors">
                                    <div class="border-2 border-gray-500 rounded-lg aspect-[2/3] border-dashed cursor-pointer"
                                        :class="userStore.theme_id == item.id 
                                        ? 'transition-all duration-150 ease-in p-2' 
                                        : 'transition-all duration-150 ease-out p-0'"
                                    >
                                        <div @click="updateTheme(item.id)" class="relative rounded-xl h-full mx-auto">
                                            <div class="absolute left-0 top-0 h-full w-full z-0" :class="item.color" />

                                            <div class="relative z-10 pt-9">
                                                <div class="rounded-full mx-auto w-12 h-12 bg-[#eff0ea] bg-opacity-70" />

                                                <div class="w-[calc(100%-20px)] mx-auto rounded-full 
                                                    h-6 mt-4 bg-[#eff0ea] bg-opacity-70" 
                                                />
                                                <div class="w-[calc(100%-20px)] mx-auto rounded-full 
                                                    h-6 mt-1 bg-[#eff0ea] bg-opacity-70" 
                                                />
                                                <div class="w-[calc(100%-20px)] mx-auto rounded-full 
                                                    h-6 mt-1 bg-[#eff0ea] bg-opacity-70" 
                                                />
