<template>
    <div id="AdminLayout" class="w-full fixed z-30">
        <div v-if="!userStore.isMobile" id="TopNav"
            class="w-full flex items-center justify-between md:pt-2.5 md:px-2.5"
        >
            <div class="flex items-center justify-between md:rounded-full md:shadow-sm 
                px-2 md:pl-6 pl-4 py-[8px] bg-white w-full border-b"
            >
                <div class="flex items-center justify-start w-full max-w-[500px]">
                    <NuxtLink to="/admin">
                        <img class="w-[23px] min-w-[23px] select-none" src="~/assets/image/linktree-logo-icon.png" />
                    </NuxtLink>

                    <div v-for="link in links" class="lg:px-2.5 px-0.5 md:block hidden">
                        <NuxtLink :to="link.url" 
                            class="flex items-center text-sm font-semibold px-1.5 py-3 rounded-lg hover:bg-gray-200"
                        >
                            <Icon :name="link.icon" class="mr-0.5" size="18" 
                ...
                <div class="flex items-center justify-center gap-3">
                    ...
                    <button class="flex items-center font-semibold px-3 py-2 rounded-full border hover:bg-gray-200">
                        <Icon name="teenyicons:upload-solid" class="mr-0.5" size="22" /> Share
                    </button>

                    <button @click="!isTopNav ? openMenu('TopNav') : isTopNav = false"
            ...
            <div v-if="isTopNav"
                class="absolute md:block hidden right-4 top-16 border shadow-[0_5px_15px_15px_rgba(0,0,0,0.1)] 
                    bg-white w-full max-w-[300px] rounded-2xl"
            >
                <button @click="logout()" 
                    class="w-full flex items-center text-gray-600 font-semibold text-[15px] p-3 hover:text-black"
                >
                    <Icon name="circum:logout" class="mr-2" /> Sign out
                </button>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between gap-4 shadow-sm bg-white w-full">
                <div class="flex w-full">
                    <div v-for="link in linksSecondaryNav" class="w-1/4">
                        <NuxtLink :to="link.url"
                            class="relative flex justify-center border-t-black text-sm w-full h-full 
                                font-semibold px-1.5 my-[1px] py-[1px] hover:bg-gray-200"
                        ...
                        >
                            <button class="relative pt-[6px]">
                                ...
                                <img v-else class="rounded-full min-w-[22px] w-[22px]" :src="link.img" />

                                <div class="relative text-[13px] text-[#676b5f]"
        ...
        <div v-if="!userStore.isMobile" id="PreviewButton"
            class="fixed bottom-10 w-full flex items-center justify-center"
        >
            <button @click="userStore.isPreviewOverlay = true"
                class="md:hidden flex items-center text-[17px] font-semibold px-5 py-2.5 bg-[#dfe2d9] rounded-full"
            >
                <Icon name="icon-park-outline:preview-open" size="20" class="mr-2" /> Preview 
            </button>
        </div>
            ...
            <div class="flex items-center justify-between w-full py-[8px]">
                ...
                <div class="flex items-center justify-between gap-3">
                    <button class="flex items-center text-[18px]  font-semibold px-3 py-2 rounded-full bg-gray-200">
                        Upgrade <Icon name="mdi:lightning-bolt" class="ml-1" size="20" />
                    </button>

                    <button class="flex items-center font-semibold px-3 py-2.5 rounded-full hover:bg-gray-200">
    ...
    >
        <div class="flex w-full">
            <div v-for="link in linksMobile" class="w-1/5">
                <button class="relative flex justify-center text-sm w-full 
                    h-full font-semibold px-1.5 py-1 hover:bg-gray-200"
                ...
                >
                    <NuxtLink :to="link.url" class="relative h-[35px]">
                        ...
                        <img v-else class="rounded-full min-w-[25px] w-[25px] mt-[2px]" :src="link.img" />

                        <div class="relative text-[13px]" :class="link.img ? '-left-[4px]' : ''">
                            <span :class="route.fullPath == link.url ? 'text-[#000000]' : '#676b5f'">
                                {{ link.name }}
                            </span>
                        </div>
                    </NuxtLink>
